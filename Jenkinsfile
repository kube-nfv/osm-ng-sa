...
pipeline {
  ...
  parameters {
    // Core GitLab / multibranch inputs
    string(name: 'GITLAB_PROJECT', defaultValue: 'osm/NG-SA', description: '')
    ...
    // E2E Tests Parameters
    string(name: 'MODULE_NAME', defaultValue: 'NG-SA', description: 'Name of the module under test')
    booleanParam(name: 'DO_ROBOT', defaultValue: true, description: 'If true, trigger downstream robot tests for merge requests')

    // Image name for build and push for promotion
    string(name: 'IMAGENAME', defaultValue: 'opensourcemano/airflow', description: 'Image name for publish')
  }
  environment {
    ...
    stage('CI') {
      ...
      stages {
        ...
        stage('E2E Test (robot)') {
          when { expression { return params.DO_ROBOT && env.JOB_BASE_NAME.contains('MR') } }
          steps {
            script {
              def dowstreamJob = "osm-e2e/${params.GITLAB_TARGET_BRANCH ?: 'master'}"
              build job: dowstreamJob,
                parameters: [
                  string(name: 'DO_IMAGE_PROMOTION_TO_DOCKERHUB', value: 'false'),
                  string(name: 'GITLAB_BRANCH', value: params.GITLAB_TARGET_BRANCH),
                  string(name: 'MODULE_NAME', value: params.MODULE_NAME ?: 'NG-SA'),
                  string(name: 'MODULE_BUILT_TAG', value: env.BUILT_TAG),
                  string(name: 'ROBOT_TAG_NAME', value: 'sanity')
                ]
            }
          }
        }
